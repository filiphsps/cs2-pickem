/**
 * @module commands/predict
 * @description The predict command.
 */
import {
    createPickEmClient,
    enrichLayout,
 ...
 * The predict command.
 * @remarks
 * This command allows the user to make predictions for a tournament.
 * @example
 * cs2-pickem predict <event-id> --interactive
 */
export const predictCommand = new Command("predict")
    ...
    .action(
        async (
            eventId: string,
            options: { section?: string; group?: string; interactive?: boolean }
        ) => {
            try {
                ...
                const [layout, items, inventory] = await Promise.all([
                    ...
                    client.getInventory(config.steamId),
                ]);
                ...
                if (options.interactive) {
                    ...
                    const openSections = enrichedLayout.sections.filter((s) =>
                        s.groups.some((g) => g.picksAllowed)
                    );
                    ...
                    const teamSticker = items.items.find(
                        (item) => item.type === "team" && item.teamId === teamChoice
                    );
                    ...
                    console.log(
                        chalk.yellow("Use --interactive flag to make predictions interactively")
                    );
