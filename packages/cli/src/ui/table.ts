...
export const displayPredictions = (layout: TournamentLayout, predictions: UserPredictions): void => {
  console.log(chalk.bold("\n📊 Your Pick'em Predictions\n"))

  if (predictions.predictions.length === 0) {
    console.log(chalk.yellow("No predictions made yet."))
    console.log(chalk.gray("Use 'cs2-pickem predict <event-id> --interactive' to make predictions"))
    return
  }
  ...
  for (const section of layout.sections) {
    ...
    const table = new Table({
      head: [chalk.white("Match"), chalk.white("Your Pick"), chalk.white("Status"), chalk.white("Points")],
...
export const displayScore = (score: BracketScore, coinTier: CoinTier): void => {
  ...
  if (score.sectionScores.length > 0) {
    ...
    const table = new Table({
      head: [chalk.white("Stage"), chalk.white("Points"), chalk.white("Correct"), chalk.white("Accuracy")],
      colWidths: [25, 10, 12, 12],
    })

    for (const section of score.sectionScores) {
      const accuracy =
        section.totalPicks > 0 ? `${Math.round((section.correctPicks / section.totalPicks) * 100)}%` : "-"
  ...
  const nextTierInfo = tierThresholds.find((t) => t.min > score.totalPoints && t.tier !== coinTier)
