import type { MarketUrlParams } from "../types.js";
...
export const buildMarketUrl = (params: MarketUrlParams): string => {
    const tournamentName =
        TOURNAMENT_NAMES[params.tournamentId] ?? `Tournament ${params.tournamentId}`;
    ...
    if (params.type === "team" && params.teamId) {
        // Search for team sticker
        searchParams.set("q", `${tournamentName}`);
        // Note: Team name should be appended based on team ID lookup
    } else if (params.type === "player" && params.playerTag) {
        // Search for player autograph sticker
