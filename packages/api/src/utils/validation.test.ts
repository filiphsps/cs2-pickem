...
describe("validateUserAuthParams", () => {
    ...
    it("should reject invalid steamId", () => {
        expect(() => validateUserAuthParams({ ...validParams, steamId: "invalid" })).toThrow(
            ValidationError
        );
    });
});
...
describe("validateUploadPredictionParams", () => {
    ...
    it("should reject invalid sectionId", () => {
        expect(() => validateUploadPredictionParams({ ...validParams, sectionId: 0 })).toThrow(
            ValidationError
        );
    });

    it("should reject empty itemId", () => {
        expect(() => validateUploadPredictionParams({ ...validParams, itemId: "" })).toThrow(
            ValidationError
        );
    });
});
...
describe("validateUploadLineupParams", () => {
    ...
    it("should reject lineup with wrong player count", () => {
        expect(() =>
            validateUploadLineupParams({ ...validParams, lineup: validLineup.slice(0, 4) })
    ...
    it("should reject lineup with invalid player data", () => {
        const invalidLineup = validLineup.map((p, i) =>
            i === 0 ? { pickId: 0, itemId: "item" } : p
        );
        expect(() => validateUploadLineupParams({ ...validParams, lineup: invalidLineup })).toThrow(
            ValidationError
        );
    });
});
